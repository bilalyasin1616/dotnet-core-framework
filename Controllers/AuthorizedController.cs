...
namespace Framework.Controllers
{
    ...
    public abstract class AuthorizedController : ControllerBase
    {
        ...
        protected ActionResult<ResponseBase> GetSearchResponse(SearchResponse data)
        {
            if (data.TotalRecords > 0)
                return Ok(ResponseModel<SearchResponse>.GetSuccess($"Returning search result", data));
            else
                return NoContent();
        }
